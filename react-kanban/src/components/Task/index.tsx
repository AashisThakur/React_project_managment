    ...
    return (
        <div ref={provided.innerRef}{...provided.draggableProps}{...provided.dragJandleProps} className="w-full cursor-grab bg-white flex flex-col justify-between gap-3 items-start shadow-sm rounded-x1 px-3 py-4 ">
            ...
            <div className="flex items-center gap-2 ">
                {tags.map((tags) => (
                    <span key={tags.title} className="px-[10px] py-[2px] text-[13px] font-medium rounded-md">
                        {tags.title}
                    </span>))}
            </div>
            <div className="w-full flex items-start flex-col ">
                <span className="text-[15.5px] font-medium text-[#555]">{title}</span>
                <span className="text-[15.5px] text-gray-500">{description}</span>
            </div>
            ...
            <div className="w-full flex items-center justify-between">
                ...
                <div className={`w-[60px] rounded-full h-[5px] ${priority === "high" ? 'bg-red-500' : priority === "medium" ? 'bg-orange-500' : 'bg-blue-500'}`} />
            </div>
        </div>
    )
}
